...
class EarlyStopping:
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpoint.pt'):
        ...
        self.counter = 0 # 안졿은 상황을 counting
        self.bet_score = None # 좋은 값
        self.early_stop = False # early stop 할지 말지 여부
        self.val_loss_min = np.Inf # validation data의 loss 최소값 (기준이 됨)
        self.delta = delta
        self.path = path
...
def train_model(model, patience, n_epochs):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for x_data, target in valid_loader:
            output = model(x_data)
            loss = loss_func(output, target) # loss값 누적
...
plt.axvline(minpos, linestyle='--', color='r', label='Early stopping check')
plt.xlabel('epochs')
plt.ylabel('loss')
plt.ylim(0, 0.5)
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefid('loss_plot.png', bbox_inches='tight')
plt.show()